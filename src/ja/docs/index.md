...
「特定完了」とするのは、二つが揃ったときだけです。アーカイブが今もその主張を支持していること。そして、そこから組んだモデルについてのアーカイブ自身の判定が「機体の挙動を再現した」と述べていること。それ以外はすべて「調査中」で、まだ開いている理由を横に置いています。

C++ の実装例を載せるのは特定完了の主張だけで、手書きではなくモデルのファイルから生成しています。その手続き全体は[アルゴリズムをどう同定しているか](/ja/docs/identifying-an-algorithm)にあります。

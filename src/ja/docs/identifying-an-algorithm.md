...
### その実装例について、何を検査していて何を検査していないか

コードが載っていると、読み手には見ただけでは答えられない問いが立ちます。これは確定稿なのか、それとも黙って壊れているのか。検査しているのは 3 つ、そして重要なもの 1 つを検査していません。

**コンパイルが通ること。** 公開している例はすべて、毎回の公開時にコンパイルしています。警告ではなくビルドを落とす検査です。

**同じ法則が返す値を返すこと。** 検査は生成された各関数をバイトの 128 設定すべてで走らせ、その結果を、ファイルに印字されている定数そのものを別の言語で評価した値と突き合わせます。両辺は同じ入力に同じ算術を掛けているので、残るのは `exp` と `log` の二つの実装の差だけです。許容値はそこに合わせてあり、印字精度に合わせたものではありません。

**その上の図が描いている値を返すこと。** それらの値を、図が丸めるのと同じ丸め方に通したものが、描かれている点と厳密に一致すること。これが頁から読み手への実際の約束です——あの曲線は、このコードが計算する値です。

**検査していないのは、最も肝心な比較、すなわち実機との比較です。** その比較はアーカイブが済ませたものであり、比べた相手は **モデル** であって、このコードではありません。コードはそのモデルをこのサイトが C++ に書き起こしたものであり、これを実機に流し込んで聴いた者はまだ一人もいません。主張の段階が述べているのはアーカイブの比較がどうだったかであって、このファイルについては上の 3 つを超えることは何も述べていません。

### 主張がどこに立っているかを述べる 6 語

主張の頁は、何よりも先にその段階を 1 語で述べます。その下にある 2 つの事実は別々に壊れるので、読み手にそれを組み直させないためです。**撤回** は、アーカイブがもう支持していない主張。**モデル一致** は、ユニットの挙動を再現したモデル。**回数切れ** は、許された改訂回数を使い切った主張です——これは追跡できた長さの上限であってモデルの判定ではなく、だからこそ回数切れの主張の下のモデルが読み取り値を再現していることがあり、その場合は再現したものとして描かれます。**未決着** は、組まれ、比較され、決着しなかったモデルです。それがモデルの側の問題だったのか試験の側だったのかは、横に並ぶ判定が述べています。**判定待ち** は、比較は走ったが判定がまだ付いていないもの。**モデル未作成** は、まだ何とも比較されていない主張です——これは失敗ではありません。失敗しうることがまだ何も起きていない段階だからです。

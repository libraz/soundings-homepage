...
- ディスク上のテイクがそれを保持している場合、各帯域の位相を回した角度。帯域エネルギーと位相は一つのエフェクトについての別々の事柄であり、すべての帯域で同じエネルギーを通しながらステップに対して異なる応答を返す二つの形は、前者では一つの測定結果になり、後者では二つになります

この素材を作る段階は `efx-rate`、`efx-bands`、`efx-time` です。ステレオの対を動かすバイトについては `balance`、`balance-bands`、`arrival` が加わります。いずれも保存済みのテイクを読むので、実機を占有しません。そこから*読み取った*ものは、機体のディレクトリには一切入りません。`inferences/` へ行きます。`inferences/` はこれらの記録を引用し、これらの記録からは引用されません。
...
- **何らかのモデル**。アルゴリズムを名指しせず、回路構成を当てはめず、係数を推定しません。測り切ったディレクトリでも、`data/` のどのディレクトリでも同じです。機体を測り切ったと言えるのは、そこからモデルを導ける状態になったときであって、導き終えたときではありません。そして導いたものは `inferences/` にあり、そこを消しても機体の記録は何も変わりません。
